    ...
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 5; ++j) {
            ...
            if (checkBound(start.x(), start.y()) && checkBound(end.x(), end.y())) {
                int blackChess = 0;
                int whiteChess = 0;
                for (int k = 0; k < 5; ++k) {//对所有情况5个位置进行判断
...
void GoBangAI::init_stats() {
    memset(stats, 0, sizeof(stats));//全部设为0
    //白连5,ai赢
    ...
    if (depth == 0 || EVAL.result != R_DRAW) {//抵达最深层/如果模拟落子可以分出输赢，那么直接返回结果，不需要再搜索
       ...
       //qDebug()<<"白方决策！";
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            sameBoard[P.pos[i].x()][P.pos[i].y()] = WHITE;//模拟己方落子,不能用board,否则可能改变board的信息
        ...
        Points P = seekPoints(rBoard);          //计算黑棋的最佳位置,需要将棋子颜色反转
...
bool GoBangAI::analyse_kill(int(*board)[15], int depth) {
    ...
    if (depth == 0 || EVAL.result != R_DRAW) {
        if (depth == 0) {
            ...
            //抵达最深层时走当前白棋得分最高的位置,此时若白棋没能取胜则返回false（输）
